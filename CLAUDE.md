...
- **Lines ~468**: `get_effort_level()` — reads reasoning effort from `CLAUDE_CODE_EFFORT_LEVEL` env var, `~/.claude/settings.json` `effortLevel` key, or defaults to `"high"`
- **Lines ~488**: `get_git_branch()` — subprocess call to `git branch --show-current`, strips ESC characters from output
